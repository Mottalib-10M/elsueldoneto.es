...
export default function SalarioBruto() {
  ...
  return (
    <div className="space-y-6">
      <CampoSalario id="sb-neto" label="Neto deseado" value={netoDeseado} onChange={setNetoDeseado} min={0} max={500000} step={500} divisor={pagas} />
          ...
          <div>
            <div className="flex gap-2">
              {([14, 12] as const).map(p => (
                <button key={p} onClick={() => setPagas(p)} className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium ${pagas === p ? 'border-brand bg-brand/10 text-brand' : 'border-gray-300 text-gray-600 dark:border-gray-600 dark:text-gray-300'}`}>{p} pagas</button>
              ))}
            </div>
      ...
      {resultado && (
        <div className="space-y-4">
          <div className="rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 p-6 text-center dark:from-blue-900/30 dark:to-blue-900/10">
            <p className="text-sm font-medium text-blue-600 dark:text-blue-400">Necesitas un bruto anual de</p>
            <p className="mt-1 text-4xl font-bold text-blue-700 dark:text-blue-300">{formatEuros(resultado.brutoNecesario)}</p>
            <p className="mt-1 text-sm text-blue-600 dark:text-blue-400">para cobrar {formatEuros(netoNum)} netos al año</p>
          </div>
          <PanelResultado resultado={resultado.desglose} />
        </div>
      )}
    </div>
  );
