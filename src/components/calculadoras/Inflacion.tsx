...
export default function Inflacion({ lang = 'es' }: { lang?: 'es' | 'en' }) {
  const l = lang === 'en';
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-xl bg-gradient-to-br from-amber-50 to-amber-100 p-6 text-center dark:from-amber-900/30 dark:to-amber-900/10">
          ...
          <p className="mt-1 text-sm text-amber-600 dark:text-amber-400">{l ? `you will need in ${aniosNum} years` : `necesitarás en ${aniosNum} años`}</p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-600">
        <table className="w-full text-sm">
          <tbody>
            <tr className="border-b border-gray-100 dark:border-gray-700"><td className="px-4 py-2 text-gray-600 dark:text-gray-300">{l ? 'Amount today' : 'Cantidad hoy'}</td><td className="px-4 py-2 text-right tabular-nums font-medium">{formatEuros(cantidadNum)}</td></tr>
            <tr className="border-b border-gray-100 dark:border-gray-700"><td className="px-4 py-2 text-gray-600 dark:text-gray-300">{l ? `Equivalent purchasing power in ${aniosNum} years` : `Poder adquisitivo equivalente en ${aniosNum} años`}</td><td className="px-4 py-2 text-right tabular-nums font-medium text-red-600">{formatEuros(resultado.valorReal)}</td></tr>
            <tr className="border-b border-gray-100 dark:border-gray-700"><td className="px-4 py-2 text-gray-600 dark:text-gray-300">{l ? 'Cumulative loss' : 'Pérdida acumulada'}</td><td className="px-4 py-2 text-right tabular-nums font-medium text-red-600">{formatPercent(resultado.perdidaPoder / cantidadNum)}</td></tr>
            <tr><td className="px-4 py-2 text-gray-600 dark:text-gray-300">{l ? 'Total cumulative inflation' : 'Inflación total acumulada'}</td><td className="px-4 py-2 text-right tabular-nums font-medium">{formatPercent((resultado.equivalenteActual - cantidadNum) / cantidadNum)}</td></tr>
          </tbody>
        </table>
      </div>
    </div>
