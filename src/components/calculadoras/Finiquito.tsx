  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <CampoEntrada id="fq-salario" label="Salario bruto mensual" value={salario} onChange={setSalario} min={0} max={50000} step={100} suffix="€/mes" />
        <CampoEntrada id="fq-dias" label="Días trabajados (último mes)" value={diasTrabajados} onChange={setDiasTrabajados} min={0} max={30} step={1} suffix="días" />
        <CampoEntrada id="fq-vacaciones" label="Días vacaciones pendientes" value={diasVacaciones} onChange={setDiasVacaciones} min={0} max={30} step={1} suffix="días" />
        <CampoEntrada id="fq-antiguedad" label="Antigüedad en la empresa" value={antiguedad} onChange={setAntiguedad} min={0} max={50} step={0.5} suffix="años" />
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Pagas extras</label>
          <select
            value={pagasExtra}
          ...
          >
            <option value={2}>2 pagas extras (14 pagas)</option>
            <option value={0}>Prorrateadas (12 pagas)</option>
          </select>
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Tipo de extinción</label>
          <select
            value={tipoDespido}
            onChange={e => setTipoDespido(e.target.value as TipoDespido)}
            className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
          >
            <option value="voluntario">Baja voluntaria</option>
            <option value="objetivo">Despido objetivo (20 d/año)</option>
            <option value="improcedente">Despido improcedente (33 d/año)</option>
            <option value="fin-contrato">Fin de contrato temporal (12 d/año)</option>
          </select>
        </div>
      </div>

      {pagasExtra === 2 && (
        <CampoEntrada id="fq-meses-paga" label="Meses desde última paga extra" value={mesesPaga} onChange={setMesesPaga} min={0} max={6} step={1} suffix="meses" helpText="Para calcular la parte proporcional" />
      )}
      ...
      <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 p-8 text-center dark:from-emerald-900/30 dark:to-emerald-900/10">
        <p className="text-sm font-medium text-emerald-600 dark:text-emerald-400">Finiquito bruto total</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-700">
              ...
              <th className="px-4 py-2 text-right font-medium text-gray-500">Importe</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-t border-gray-100 dark:border-gray-700">
              <td className="px-4 py-2 text-gray-600 dark:text-gray-300">Salario pendiente ({diasNum} días)</td>
              <td className="px-4 py-2 text-right tabular-nums font-medium">{formatEuros(resultado.salarioPendiente)}</td>
            </tr>
            {pagasExtra === 2 && (
              <tr className="border-t border-gray-100 dark:border-gray-700">
                <td className="px-4 py-2 text-gray-600 dark:text-gray-300">Pagas extras proporcionales</td>
                <td className="px-4 py-2 text-right tabular-nums font-medium">{formatEuros(resultado.pagasExtrasProporcionales)}</td>
              </tr>
            )}
            <tr className="border-t border-gray-100 dark:border-gray-700">
              <td className="px-4 py-2 text-gray-600 dark:text-gray-300">Vacaciones pendientes ({diasVacNum} días)</td>
            ...
            {resultado.indemnizacion > 0 && (
              <tr className="border-t border-gray-100 dark:border-gray-700">
                <td className="px-4 py-2 text-gray-600 dark:text-gray-300">Indemnización</td>
            ...
            <tr className="border-t-2 border-gray-300 dark:border-gray-600">
              <td className="px-4 py-2 font-bold text-charcoal dark:text-gray-100">Total bruto</td>
