  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <CampoEntrada id="pr-capital" label="Capital del préstamo" value={capital} onChange={setCapital} min={0} max={1000000} step={500} suffix="€" />
        <CampoEntrada id="pr-tin" label="TIN anual" value={tin} onChange={setTin} min={0} max={50} step={0.1} suffix="%" helpText="Tipo de Interés Nominal" />
        <CampoEntrada id="pr-plazo" label="Plazo" value={plazo} onChange={setPlazo} min={1} max={600} step={1} suffix="meses" />
      </div>
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 p-6 text-center dark:from-gray-700 dark:to-gray-800">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total a devolver</p>
              <p className="mt-1 text-2xl font-bold text-charcoal dark:text-gray-100">{formatEuros(resultado.totalPagado)}</p>
            </div>
          </div>

          <div>
            <h3 className="mb-3 text-lg font-semibold text-charcoal dark:text-gray-100">Tabla de amortización</h3>
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-600">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-700">
                    <th className="px-3 py-2 text-left font-medium text-gray-500">Mes</th>
                    <th className="px-3 py-2 text-right font-medium text-gray-500">Cuota</th>
                    <th className="px-3 py-2 text-right font-medium text-gray-500">Capital</th>
                    <th className="px-3 py-2 text-right font-medium text-gray-500">Intereses</th>
                    <th className="px-3 py-2 text-right font-medium text-gray-500">Pendiente</th>
                  </tr>
                </thead>
                <tbody>
            ...
            {plazoNum > 12 && <p className="mt-2 text-xs text-gray-500">Mostrando los primeros 12 de {plazoNum} meses.</p>}
          </div>
        </>
      )}
    </div>
